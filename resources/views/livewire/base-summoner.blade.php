      ...
      <svg class="h-3 w-3 text-indigo-600" fill="currentColor" viewBox="0 0 12 12">
        <path
	        d="M3.707 5.293a1 1 0 00-1.414 1.414l1.414-1.414zM5 8l-.707.707a1 1 0 001.414 0L5 8zm4.707-3.293a1 1 0 00-1.414-1.414l1.414 1.414zm-7.414 2l2 2 1.414-1.414-2-2-1.414 1.414zm3.414 2l4-4-1.414-1.414-4 4 1.414 1.414z"/>
				...
				<div class="w-7/12">
					<livewire:filter :me="$summoner" :filters="$filters"></livewire:filter>
				</div>
			</div>
				...
				<div class="hidden sm:block">
					<div class="border-b border-gray-200">
						<nav class="-mb-px flex space-x-8" aria-label="Tabs">
							...
							@foreach($tabs as $key => $name)
								@if($key != "versus")
									<a href="{{route($key, ['summonerId' => $summonerId]).$this->getParamsUrl()}}">
										@endif
										<div
											@class(["border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm" => $tab != $key, "border-indigo-500 text-indigo-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm" => $tab == $key, "cursor-pointer" => $key != "versus"])>
											{{$name}}
										</div>
										@if($key != "versus")
									</a>
		...
		<div class="relative">
			<div wire:loading>
				<x-defer-loading></x-defer-loading>
			</div>

			<div>
				@if($tab == "summoner")
					<livewire:matches :me="$summoner" :version="$version"
					                  wire:key="matches_{{json_encode($filters ?? [])}}"
					                  :filters="$filters"></livewire:matches>
				@elseif($tab == "encounters")
					<livewire:encounters :me="$summoner" :version="$version"
					                     wire:key="encounters_{{json_encode($filters ?? [])}}"
					                     :filters="$filters"></livewire:encounters>
				@elseif($tab == 'live_game')
					<livewire:live-game :version="$version" :me="$summoner"
					                    wire:key="live_game_{{json_encode($filters ?? [])}}"></livewire:live-game>
				@elseif($tab == 'versus')
					<livewire:summoner-versus :me="$summoner" :other="$otherSummonerId"
					                          :version="$version"
					                          wire:key="summoner_versus_{{json_encode($filters ?? [])}}"
					                          :filters="$filters"></livewire:summoner-versus>
				@elseif($tab == 'champions')
					<livewire:champions :me="$summoner" :version="$version"
					                    wire:key="champions_{{json_encode($filters ?? [])}}"
					                    :filters="$filters"></livewire:champions>
				@endif
			</div>
