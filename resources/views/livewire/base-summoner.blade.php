                ...
                <div class="hidden sm:block">
                    <div class="border-b border-gray-200">
                        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                            ...
                            @foreach(\App\Enums\TabEnum::cases() as $enum)
                                @if($enum->only_summoner_id())
                                    <a href="{{route($enum->value, ['summonerId' => $summonerId]).$this->getParamsUrl()}}">
                                        @endif
                                        <div
                                            @class(["border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm" => $tab != $enum, "border-indigo-500 text-indigo-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm" => $tab == $enum, "cursor-pointer" => $enum->only_summoner_id()])>
                                            {{$enum->title()}}
                                        </div>
                                        @if($enum->only_summoner_id())
                                    </a>
