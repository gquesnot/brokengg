...
{{--						@endforeach--}}
{{--					</select>--}}
					<x-form.select2 model="filters.queue" :options="$options['queue']" :nullable="true" placeholder="Queue" />
...
{{--						@endforeach--}}
{{--					</select>--}}
					<x-form.select2 model="filters.champion" :options="$options['champion']" :opt-group="true" :nullable="true" placeholder="Champion" />
				</div>
			</div>
			<div class="flex items-end my-2 ">
				<div class="w-60 flex">
				...
				<div class="">
					<x-input-toggle model="filters.filterEncounters" :value="$filters['filterEncounters']"></x-input-toggle>
				</div>
			</div>
		</div>
		<div class="w-1/2 flex flex-col">
			...
			<div>
				@error('filters.dateStart')<x-alert :message="$message" type="error"></x-alert> @enderror
				@error('filters.dateEnd')<x-alert :message="$message" type="error"></x-alert> @enderror
				@error('filters.queue')<x-alert :message="$message" type="error"></x-alert> @enderror
				@error('filters.champion')<x-alert :message="$message" type="error"></x-alert> @enderror
			</div>
